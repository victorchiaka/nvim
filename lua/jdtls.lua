...
local function get_jdtls()
  ...
  local config = jdtls_path .. "/config_" .. SYSTEM -- ( .. concatinating in lua )
...
local function setup_jdtls()
  ...
  local root_dir = jdtls.setup({
    cmd = { 'java-lsp.sh' }
  }).find_root({ '.git', 'mvnw', 'gradlew', 'pom.xml', 'build.gradle' });
  ...
  local function java_keymaps()
    ...
    vim.keymap.set('v', '<leader>jtm', "<Esc><Cmd> lua require('jdtls').test_nearest_method(true)<CR>",
      { desc = "[J]ava [T]est Method" })
    -- Set a Vim motion to <Space> + <Shift>J + <Shift>T to run an entire test suite (class)
    vim.keymap.set('n', '<leader>jtc', "<Cmd> lua require('jdtls').test_class()<CR>", { desc = "[J]ava [T]est Class" })
